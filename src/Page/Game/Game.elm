module Page.Game.Game exposing (Model, Msg, update, view, init)
...
update msg model =
  case  msg of
    PlayGame pgmsg ->
        let
            (pgm2, pgmsg2) =  PlayGame.update pgmsg model.gameModel
        in
            ({model | gameModel =  pgm2  }, PlayGame pgmsg2)
          --  (PlayGameModel pgm2, PlayGame pgmsg2)
    ShowsManage smsg ->
        let
            ( sm2 , smsg2) = ShowsManage.update smsg model.showsModel
        in
...
view model =
    case model of
        PlayGameModel pgModle ->
            PlayGame.view pgModle
        ShowsManageModel smodel ->
            ShowsManage.view smodel
